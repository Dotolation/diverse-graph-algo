...
public class DiverseShortestPaths {
	...
	public Graph<String, DefaultWeightedEdge> edgeRemoval(Graph<String, DefaultWeightedEdge> g, String source, String target){
	...
	public GraphPath<String, DefaultWeightedEdge> getkPaths(String source, String target, int k){
		...
		Function<DefaultWeightedEdge, Double> arcCost = edge -> g.getEdgeWeight(edge);
		Function<String, Integer> supplyDemand = vertex -> {
			
			if(vertex.equals(source)) {
				
				return stPathsSize * k;
		...
		MinimumCostFlowProblem<String, DefaultWeightedEdge> problem = new MinimumCostFlowProblemImpl<>(g, supplyDemand, maxCapacity);  
		
		MinimumCostFlowAlgorithm<String, DefaultWeightedEdge> solver = new CapacityScalingMinimumCostFlow<>();
		
		solver.getMinimumCostFlow(problem).getFlowMap();
